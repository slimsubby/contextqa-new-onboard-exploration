...
interface CQARobotProps {
  className?: string;
  animate?: boolean;
}

export default function CQARobot({ className = "", animate = true }: CQARobotProps) {
  ...
  useEffect(() => {
    ...
    if (!robot || !rightArm || !leftArm || !head || !leftEye || !rightEye) return;

    const ctx = gsap.context(() => {
      gsap.set(rightArm, { svgOrigin: "143 144" });
      gsap.set(head,     { svgOrigin: "109 115" });
      gsap.set(leftArm,  { svgOrigin: "74 138"  });

      gsap.fromTo(leftArm,
        { rotation: -5 },
        { rotation: 5, duration: 1.9, ease: "sine.inOut", yoyo: true, repeat: -1 }
      );

      gsap.timeline({ repeat: -1, repeatDelay: 5, defaults: { ease: "sine.inOut" } })
        .to(head, { rotation: 3, duration: 0.9 })
        .to(head, { rotation: 6, duration: 0.7, delay: 0.4 })
        .to(head, { rotation: 0, duration: 1.0, delay: 0.6 });
      ...
      gsap.timeline({ repeat: -1, repeatDelay: 1.5 })
        .to(rightArm, { rotation: -10, duration: 0.22, ease: "sine.inOut" })
        .to(rightArm, { rotation:  10, duration: 0.28, ease: "sine.inOut" })
        .to(rightArm, { rotation: -10, duration: 0.28, ease: "sine.inOut" })
        .to(rightArm, { rotation:  10, duration: 0.28, ease: "sine.inOut" })
        .to(rightArm, { rotation: -10, duration: 0.28, ease: "sine.inOut" })
        .to(rightArm, { rotation:   0, duration: 0.22, ease: "sine.inOut" });

      const eyes = [leftEye, rightEye];
      function scheduleBlink() {
        gsap.timeline({ onComplete: scheduleBlink, delay: 2 + Math.random() * 2.5 })
          .to(eyes, { opacity: 0, duration: 0.18, ease: "power2.inOut" })
          .to(eyes, { opacity: 1, duration: 0.22, ease: "power2.inOut" });
      }
      scheduleBlink();
    });
